...
!  integer, parameter :: wp = selected_real_kind(p=14)
  ...
  ! system variables
  integer :: dnum, dcols
  real(wp), allocatable, dimension(:) :: xdat, ydat, edat, wdat, rdat
  real(wp), dimension(16) :: yr_mean, T_mean, T_wei, T_sig, T_ones
  logical, allocatable, dimension(:) :: skipdat
  logical, dimension(16) :: T_skip
  character(64), allocatable, dimension(:) :: colnames
  real(wp), dimension(5) :: linparams
